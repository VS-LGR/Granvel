...
export function Textarea({ id, label, hint, fieldVariant = "default", className = "", ...props }: TextareaProps) {
  ...
  const controlClass =
    fieldVariant === "onDark"
      ? "min-h-[120px] rounded-[var(--radius-md)] border border-zinc-400/90 bg-white px-3 py-2.5 text-base text-zinc-900 shadow-sm placeholder:text-zinc-500 focus:border-[var(--color-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/30 [color-scheme:light]"
      : "min-h-[120px] rounded-[var(--radius-md)] border border-[var(--color-line)] bg-white px-3 py-2.5 text-base text-zinc-900 placeholder:text-zinc-500 focus:border-[var(--color-accent)] focus:outline-none [color-scheme:light]";
