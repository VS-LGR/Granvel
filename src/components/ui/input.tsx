...
export function Input({ id, label, hint, fieldVariant = "default", className = "", ...props }: InputProps) {
  ...
  const controlClass =
    fieldVariant === "onDark"
      ? "rounded-[var(--radius-md)] border border-zinc-400 bg-white px-3 py-2 text-zinc-900 shadow-sm placeholder:text-zinc-500 focus:border-[var(--color-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/30 [color-scheme:light]"
      : "rounded-[var(--radius-md)] border border-[var(--color-line)] bg-white px-3 py-2 text-zinc-900 shadow-inner placeholder:text-zinc-500 focus:border-[var(--color-accent)] focus:outline-none [color-scheme:light]";
